...
export const LOAD_ENVS = (req = false): ENV_TYPES => {
  return {
    ...
    JWT_EXPIRY_TIME: parseInt(
      tryReadEnv(ENV_NAMES.JWT_EXPIRY_TIME, req, `${ENV_DEFAULT.JWT_EXPIRY_TIME}`),
    ),
    JWT_REFRESH_EXPIRY_TIME: parseInt(
      tryReadEnv(ENV_NAMES.JWT_REFRESH_EXPIRY_TIME, req, `${ENV_DEFAULT.JWT_REFRESH_EXPIRY_TIME}`),
    ),
